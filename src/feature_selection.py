...
import pandas as pd
from modelxgb import load_and_prepare_data, train_model, print_metrics
from scipy.stats import chi2_contingency
from genes import GENES


def select_important_genes_feature_matrix(
    feature_matrix, disease_df, gene_prefix="t_alt_", top_n=50, verbose=True
):
...
def main():
    ...
    for top_n in ns:
        ...
        selected_genes = select_important_genes_feature_matrix(
            mutation_data_full,
            disease_data,
            gene_prefix="t_alt_",
            top_n=top_n,
        ...
        print(
            f"Cases with feature selection: {n_cases} - total cases: {len(mutation_data_full)}"
        )
        results.append({"top_n": top_n, "n_cases": n_cases, **model_info})
