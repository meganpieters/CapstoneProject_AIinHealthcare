    ...
    for fp, case in tqdm(files):
        ...
        grp = (
            ...
            .agg(
                ...
                mean_vaf=("vaf", "mean"),
                worst_class=("Variant_Classification", lambda x: x.mode().iat[0]),
            )
            .reset_index()
        )
        grp["case"] = case
...
def make_feature_matrix(long_df, genes):
    # Numeric features
    feat_counts = long_df.pivot(
        index="case", columns="Hugo_Symbol", values=["t_alt", "mean_vaf"]
    ).fillna(0)
...
def load_mutation_data():
    if not os.path.exists("combined_mutation_matrix.csv"):
        raise FileNotFoundError(
            "combined_mutation_matrix.csv does not exist. Please run the data processing script first."
        )
    df = pd.read_csv("combined_mutation_matrix_full.csv")
...
def load_disease_data():
    if not os.path.exists("disease_ids.csv"):
        raise FileNotFoundError(
            "disease_ids.csv does not exist. Please run the disease ID script first."
        )
